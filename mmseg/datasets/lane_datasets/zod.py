    ...
    def load_annotations(self):
        ...
        with open(self.data_list, 'r') as anno_obj:
            ...
            for k, id in enumerate(all_ids):
                image_folder = f'data/zod_dataset/single_frames/{id}/camera_front_blur'
                image_path = os.path.join(image_folder,  next(iter(os.listdir(image_folder)), None))
                anno_path = '/'.join(image_path.split('/')[0:2]+ ['cache_dense']+ image_path.split('/')[3:])[:-3]+'pkl'
