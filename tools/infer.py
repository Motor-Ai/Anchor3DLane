...
def python_nms(
    proposals, nms_thres=0, conf_threshold=None, refine_vis=False, vis_thresh=0.5
):
    softmax = nn.Softmax(dim=1)

    scores = (
        1 - softmax(proposals[:, 5 + 20 * 3 : 5 + 20 * 3 + 21])[:, 0]
    )  # pos_score  # for debug
...
if __name__ == "__main__":
    ...
    for i in images_path:
        img = load_image(i)
        resized_img = transform_img(img)

        result = model(resized_img, batch_size)
        proposals = torch.tensor(result[0].reshape(1, -1, 86))
        print("Proposals shape before nms: ", proposals.shape)

        # Python output
        python_out = python_nms(proposals[0], 2, 0.2, True, 0.5).numpy()
        print("ouput after nms from python:  ", len(python_out))
